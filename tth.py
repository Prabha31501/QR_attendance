...
def entry():
    ...
    def process_attendance(decoded_objects, current_date, current_time):
        if decoded_objects:
            ...
            if not is_deactivated(compare_number):
                # Check attendance
                if update_out_time(compare_number, current_date, current_time):
    ...
    def update_out_time(compare_number, current_date, current_time):
    ...
    def display_attendance_info(data, current_date, current_time, entry_type):
        if hasattr(root, 'entry_out_label'):
            root.entry_out_label.destroy()

        entry_frame.update()
        entry_out_label = Label(entry_frame, text=f"Today {entry_type} Entry", bg="white",font=('Timesnewroman', 20, 'bold'))
        entry_out_label.place(x=118, y=40)
        entry_frame.entry_out_label = entry_out_label
        ...
        emp_label = Label(entry_frame, text="Emp.Id  :" + data[3], bg="white", font=('Timesnewroman', 14, 'bold')).place(x=40, y=220)
        date_label = Label(entry_frame, text="Date  : " + current_date, bg="white",font=('Timesnewroman', 14, 'bold')).place(x=40, y=260)
        In_label = Label(entry_frame, text=f"In - Time  : {current_time if entry_type == 'In-Time Entry' else ''}",bg="white", font=('Timesnewroman', 14, 'bold')).place(x=40, y=300)
        Out_label = Label(entry_frame, text=f"Out - Time  : {current_time if entry_type == 'Out-Time Entry' else ''}", bg="white", font=('Timesnewroman', 14, 'bold')).place(x=40, y=340)
        root.after(5000, back)

    # Call the process_attendance function with the required parameters
